...
use crate::parser::{NodePosition, Parser};
use crate::{unwrap_some, Result};
use crate::interpreter::{Expr, Literal, BinaryOperator, UnaryOperator};
// use log::trace;
...
impl<'a> Parser<'_> {
    pub fn parse_expression(&mut self) -> Result<Expr<'a>> {
        ...
        let mut l_value = match self.advance().type_ {
            ...
            TokenType::True=>{
                Expr::Literal(Literal::Boolean(true).into())
            }
            TokenType::False=>{
                Expr::Literal(Literal::Boolean(false).into())
            }
            TokenType::String(s)=>{
                Expr::Literal(Literal::String(std::borrow::Cow::Owned(s)).into())
            }
            TokenType::Identifier(i)=>{
                Expr::Variable(std::borrow::Cow::Owned(i))
            }
            TokenType::LBrack =>{
                self.parse_array().unwrap()
            }
            TokenType::If => {
                self.parse_if().unwrap()
            }
            TokenType::Let => {
                self.parse_let().unwrap()
            }
            TokenType::While => {
                ...
                match self.advance().type_ {
                    TokenType::Then => {}
                    ref x=>{
                        panic!("expected `then` after while-loop condition. found `{:?}`", x);
                    }
                }
            ...
            TokenType::Fn => {
                self.parse_lambda().unwrap()
            }
            TokenType::Minus=>{
                Expr::UnaryOp(UnaryOperator::Negate, Box::new(self.parse_expression().unwrap()))
            }
            TokenType::Not=>{
                Expr::UnaryOp(UnaryOperator::Not, Box::new(self.parse_expression().unwrap()))
            }
            ref x=>panic!("found {:?}. line: {}, pos: {}", x, self.line_no, self.pos),
        };
        
        loop {
            if let TokenType::LBrack = unwrap_some!(self.tokens.peek()).type_ { // array index
                self.advance(); // eat '['
        ...
        loop {
            ...
            if let TokenType::LParen = unwrap_some!(self.tokens.peek()).type_ { // call
                self.advance(); // eat '('
                let mut args = vec![];
                let index = self.parse_expression();
                loop {
        ...
        loop {
            ...
            match unwrap_some!(self.tokens.peek()).type_ {
                ...
                |TokenType::And
                |TokenType::Or =>{
                    op = tokentype_to_binop(self.advance().type_) 
                }
                _=>{
                    break;
                }
    ...
    fn parse_array(&mut self) -> Result<Expr<'a>> {
        let mut retval = Expr::Literal(Literal::Boolean(false).into());
        let mut args = vec![];
        loop {
            if unwrap_some!(self.tokens.peek()).type_ == TokenType::End {
                self.advance(); // Eat ','
    ...
    fn parse_if(&mut self) -> Result<Expr<'a>> {
        ...
        match unwrap_some!(self.tokens.peek()).type_ {
            TokenType::Then=>{},
            ref x =>return Err(format!("Expected `then` after `if` condition. found {:?}. line: {}, pos: {}", x, self.line_no, self.pos))
        }
        ...
        match unwrap_some!(self.tokens.peek()).type_ {
            TokenType::Else=>{
                ...
                return Ok(Expr::If(Box::new(cond), Box::new(if_branch), Some(Box::new(else_branch))))
            },
